-- Dropar a view primeiro (depende da função)
DROP VIEW IF EXISTS public.groups_admin;

-- Dropar a função existente para poder recriar com nova assinatura
DROP FUNCTION IF EXISTS public.get_admin_groups();

-- Recriar a função get_admin_groups com leader_email
...
-- Recriar a view groups_admin para incluir leader_email
CREATE VIEW public.groups_admin WITH (security_invoker=on) AS
SELECT 
  g.id,
  g.name,
  g.city,
